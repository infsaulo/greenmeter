...
def recommend(obj, title_intersec, desc_intersec, tag_tag, title_tag, num_rec, title_scale, desc_scale, content_scale, assoc_scale, spread_scale):
    ...
    rescore(assoc_score, assoc_scale)
    
    spread_score = compute_TS(obj, ["TITLE", "DESCRIPTION"])
    rescore(spread_score, spread_scale)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 11:
        print "usage: %s <train file> <test file> <min. support> <min. confidence> <num recomendacoes> <title scale> <desc. scale> <content scale> <assoc. scale> <spread scale>" % sys.argv[0]
        sys.exit(-1)
    ...
    assoc_scale = float(sys.argv[9])
    spread_scale = float(sys.argv[10])
    ...
    while test_object != None:
        rec = recommend(test_object, title_intersec, desc_intersec, tag_tag, title_tag, num_rec, title_scale, desc_scale, content_scale, assoc_scale, spread_scale)
