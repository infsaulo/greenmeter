...
def recommend(obj, title_intersec, desc_intersec, tag_tag, title_tag, num_rec, title_scale, desc_scale, content_scale, assoc_scale):
    ...
    return merge_scores((content_score, assoc_score))


if __name__ == "__main__":

    if len(sys.argv) != 10:
        print "usage: %s <train file> <test file> <min. support> <min. confidence> <num recomendacoes> <title scale> <desc. scale> <content scale> <assoc. scale>" % sys.argv[0]
        sys.exit(-1)
    ...
    while test_object != None:
        rec = recommend(test_object, title_intersec, desc_intersec, tag_tag, title_tag, num_rec, title_scale, desc_scale, content_scale, assoc_scale)
