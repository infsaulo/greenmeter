...
def recommend(tf, df, tagf, N, rules, num_rec, alpha, top):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 9:
        print "usage: %s <train file> <test file> <min. support> <min. confidence> <alpha> <top tfidf> <num recomendacoes> [<field name>]+" % sys.argv[0]
        sys.exit(-1)
    ...
    while test_object != None:
        ...
        rec = recommend(tf, df, tagf, n, rules, num_rec, alpha, top)
        for tag in rec:
            print tag,
        print
        test_object = get_next_concat(test_file)
