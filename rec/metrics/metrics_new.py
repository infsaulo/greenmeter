    ...
    def compute_instance_stats(self):
        ...
        while instance != None:
            ...
            if len(conc_list) != 0:
                ...
                print "%s\tCATEGORY%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f" % (ID, cat, self.media_top(spread_list), self.media_top(conc_list), self.media_top(conc_spread_list), self.media_top(iff_spread_list), self.media_top(iff_conc_list), self.media_top(conc_sp_iff_list), self.media_top(newconc_list), self.media_top(newconc_spread_list), self.media_top(iff_newconc_list), self.media_top(newconc_sp_iff_list), len(conc_list), self.media_top(ig_list), self.media_top(iff_list))
            else:
                print "EMPTY#%s\tCATEGORY%d\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0" % (ID, cat) 
            instance = get_next_object(fi_file)
