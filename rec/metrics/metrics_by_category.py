...
class Metrics:
    ...
    def compute_instance_stats(self):
        ...
        cat_stats = {}
        n_considered_instances = defaultdict(float)
        ...
        while instance != None:
            ...
            for t in instance[1]:
                ...
                newconc = conc * (1.0 * self.num_objects / fcat)
            ...
            if len(conc_list) != 0:
                if cat not in cat_stats:
                    cat_stats[cat] = [0 for x in xrange(11)]
                dic = cat_stats[cat]
                #dic["FIS"] += self.media_top(spread_list)
                #dic["FICC"] += self.media_top(conc_list)
                #dic["SPxTCC"] += self.media_top(conc_spread_list)
                #dic["IFFxSP"] += self.media_top(iff_spread_list)
                #dic["IFFxTCC"] += self.media_top(iff_conc_list)
                #dic["IFFxSPxTCC"] += self.media_top(conc_sp_iff_list)
                
                #dic["newFICC"] += self.media_top(newconc_list)
                #dic["SPxnewTCC"] += self.media_top(newconc_spread_list)
                #dic["IFFxnewTCC"] += self.media_top(iff_newconc_list)
                #dic["IFFxSPxnewTCC"] += self.media_top(newconc_sp_iff_list)
                #dic["N"] += len(conc_list)
                
                dic[0] += self.media_top(spread_list)
                dic[1] += self.media_top(conc_list)
                dic[2] += self.media_top(conc_spread_list)
                dic[3] += self.media_top(iff_spread_list)
                dic[4] += self.media_top(iff_conc_list)
                dic[5] += self.media_top(conc_sp_iff_list)
                
                dic[6] += self.media_top(newconc_list)
                dic[7] += self.media_top(newconc_spread_list)
                dic[8] += self.media_top(iff_newconc_list)
                dic[9] += self.media_top(newconc_sp_iff_list)
                dic[10] += len(conc_list)
                n_considered_instances[cat] += 1

                #print "C%d", self.media_top(spread_list), self.media_top(conc_list), self.media_top(conc_spread_list), self.media_top(iff_spread_list), self.media_top(iff_conc_list), self.media_top(conc_sp_iff_list), self.media_top(iff_list), self.media_top(newconc_list), self.media_top(newconc_spread), self.media_top(iff_newconc_list), self.media_top(newconc_sp_iff_list), len(conc_list) % cat
        ...
        for c in cat_stats:
            ...
            print c,
            for v in dic:
                print 1.0*v/n_considered_instances[c],
            print
