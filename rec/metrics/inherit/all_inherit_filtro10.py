...
def merge(objects, cats, sims, k, same_class, g, infogain, ft, fc, ftc, num_objects, num_cats):
    ...
    for i in xrange(0, len(objects)):
        ...
        for pair in sorted(candidate_terms_ig.items(), key=itemgetter(1), reverse=True):
            ...
            if n >= g:
                break
            if (t not in objects[i]) and (term_freq_neighbors[t] >= k) and (ft[t] >= 10): #inherit
                n += 1.0
                #cmax = 0
                #pmax = 0.0
...
if __name__ == "__main__":
    if len(sys.argv) != 6 and len(sys.argv) != 7:
        print "usage: %s <libsvm file> <graph file> <limiar de sim.> <limiar de consenso> <mesma classe> [g]" % sys.argv[0]
        sys.exit(-1)
    ...
    merge(objects_and_cats[0], objects_and_cats[1], s, k, same_class, g, stats[0], stats[1], stats[2], stats[3], stats[4], stats[5])
    libsvm_file.close()
