    ...
    for sample_no in xrange(SAMPLE_COUNT):
        for fold_no in xrange(FOLD_COUNT):
            ...
            try:
                ...
                NAMES = load_metrics(metrics, metrics_file)
            except:
                break
    print len(hits)
    print len(metrics)
    print len(metrics[0])
    return (hits, metrics, NAMES)


def make_hists(hits, metrics, num_bins, NAMES):
    ...
    for values in metrics:
        ...
        for i in xrange(num_bins):
            ...
            if count[i] != 0:
                ...
                acc = 0.0
            print "%f\t%f\t%d\t%d" % (x, acc, count[i], hist[i])
            #print x, acc, count[i]
...
if __name__ == "__main__":
    ...
    make_hists(data[0], data[1], 10, data[2])
