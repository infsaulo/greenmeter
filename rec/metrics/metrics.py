        ...
        while instance != None:
            ...
            for t in instance[1]:
                ...
                if  freq < self.fmin or freq > self.fmax:
                    continue
                conc = self.ftc[t][cat] / self.stats[t][0]
                spread_list.append(spread[t])
                conc_list.append(conc)
                conc_spread_list.append(conc * spread[t])
            if len(conc_list) != 0:
                print self.media_top(spread_list), self.media_top(conc_list), self.media_top(conc_spread_list), len(conc_list)
